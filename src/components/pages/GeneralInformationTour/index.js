...
function GeneralInformationTour({ languageSelected, tour, setTour, tourSchedule, setTourSchedule, isDisabled }) {
    ...
    const handleInputMinToActive = (value) => {
        if (value < 0 || value === '') {
            setTour(pre => pre)
        }
        else {
            let valueRaw = value
            if (value > parseInt(tour.maxToActive)) {
                valueRaw = tour.maxToActive
            }
            setTour({ ...tour, minToActive: valueRaw })
        }
    }

    const handleInputMaxToActive = (value) => {
        if (value < 0 || value === '') {
            setTour(pre => pre)
        }
        else {
            if (value < parseInt(tour.minToActive)) {
                setTour({ ...tour, minToActive: value, maxToActive: value })
            }
            else {
                setTour({ ...tour, maxToActive: value })
            }
        }
    }
                            ...
                            <div className="d-flex line-input">
                                <div className="mlr-50 ">
                                    ...
                                    <input onFocus={(e) => handleFocus(e.target)}
                                        ...
                                        onChange={(e) => handleInputMinToActive(e.target.value)}
                                        className="input-inline" type='number' min={1} />
                                </div>
                                <div className="mlr-50">
                                    <label className="d-block title-create-tour">{languageList.txtMaxToActive}<span className="requird-star">*</span></label>
                                    <input onFocus={(e) => handleFocus(e.target)}
                                        onBlur={(e) => handleBlur(e.target)}
                                        value={tour.maxToActive} disabled={isDisabled}
                                        onChange={(e) => handleInputMaxToActive(e.target.value)}
                                        className="input-inline" type='number' min={1} />
                                </div>
                            </div>
                        } */}
