...
import { API_DASHBOARD_REPORT, API_GET_ALL_TOUR, API_GET_BOOKING_BY_MONTH, API_GET_BOOKING_BY_MONTH_STARTDATE, API_GET_CUSTOMER, API_GET_SERVICE_BY_CONDITION } from '../../API';
...
function AdminDashboard({ languageSelected }) {
    ...
    const [countBookingThisMonth, setCountBookingThisMonth] = useState(0)
    const [countTour, setCountTour] = useState(0)
    const [countBookingLastMonth, setCountBookingLastMonth] = useState(0)
    const [countBookingTwoMonthAgo, setCountBookingTwoMonthAgo] = useState(0)
    ...
    const [fromMonth, setFromMonth] = useState(`${year}-${month - 5 < 10 ? '0' : ''}${month - 5}`)
    const [toMonth, setToMonth] = useState(monthNow)

    const [typeChart, setTypeChart] = useState(0)

    const [dataBookingDate, setDataBookingDate] = useState([
        {
            time: '06/2022',
            booking: 1
        },
    ...
    useEffect(() => {
        const monthFrom = parseInt(fromMonth.split('-')[1])
        const yearFrom = parseInt(fromMonth.split('-')[0])
        const monthTo = parseInt(toMonth.split('-')[1])
        const yearTo = parseInt(toMonth.split('-')[0])

        let stringMonth = `${monthTo}/${yearTo}`

        if (yearFrom > yearTo || (yearTo === yearFrom && monthFrom > monthTo)) {
            stringMonth = `${monthFrom}/${yearFrom}`
        }

        axios.get(optionChart ? API_GET_BOOKING_BY_MONTH : API_GET_BOOKING_BY_MONTH_STARTDATE, {
            params: {
                monthYear: stringMonth,
                numberOfMonth: Math.abs((yearTo - yearFrom) * 12 - (monthFrom - monthTo)) + 1
            }
        }).then(res => {
            ...
            for (let i = data.length - 1; i >= 0; i--) {
                let count = {
                    time: data[i].monthYear,
                    booking: data[i].thisMonth
                }
                booking.push(count)
            }

            setDataBookingDate(booking)
            setDataStartDate(booking)
        })
    }, [fromMonth, toMonth, optionChart])

    useEffect(() => {
        axios.get(API_DASHBOARD_REPORT).then(res => {
            setCountBookingThisMonth(res.data.data.thisMonth)
            setCountBookingLastMonth(res.data.data.lastMonth)
            setCountBookingTwoMonthAgo(res.data.data.beforeLastMonth)
        }).catch(e => {
            console.log(e)
        })
            ...
            <div className='d-flex space-between'>
                ...
                <div className='tag-dashboard bd-green-2' onMouseEnter={() => setShowNoteRate(true)}
                    ...
                    <div className='text-center data-number-dashboard'>
                        ...
                        {!isNaN(rate.current) ? (rate.current.toFixed() > 0 ? rate.current.toFixed() : rate.current.toFixed() * -1)
                            :
                            0}
                        %
                    </div>
            ...
            <div className='d-flex space-between btn-option-chart'>
                <div className='w-75 d-flex space-between'>
                    <div className='d-flex center-vertical'>
                        ...
                        <input value={fromMonth} onChange={(e) => setFromMonth(e.target.value)} type='month' className='w-50 font-14 ml-20 br-10 pt-10 pb-10 pl-20 select-range-month' />
                        <label className='ml-20'>-</label>
                        <input value={toMonth} onChange={(e) => setToMonth(e.target.value)} type='month' className='w-50 font-14 ml-20 br-10 pt-10 pb-10 pl-20 select-range-month' />
                    </div>
